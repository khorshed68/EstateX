...
class BuyerAuthController extends Controller
{
    ...
    public function processRegister(Request $request)
    {
        ...
        try {
            ...
            // Insert new buyer user (roleId = 3, status = pending) using raw SQL
            DB::insert("
                INSERT INTO users (id, roleId, fullname, email, password, phone, profileImage, status) 
                VALUES (:id, 3, :fullname, :email, :password, :phone, :profileImage, 'pending')
            ", [
                'id' => $nextId,
            ...
            return redirect()->route('buyer.login')->with('success', 'Your buyer account has been created successfully and is pending administrator approval.');
        } catch (\Exception $e) {
