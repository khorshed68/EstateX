...
<div class="space-y-6">
    ...
    <div class="mb-6">
        <h3 class="font-outfit font-bold text-lg text-slate-200">Manage Buyer Profile</h3>
        <p class="text-xs text-slate-500 mt-0.5">Configure your contact details, personal avatar, and password credentials.</p>
    </div>
    ...
    <form action="{{ route('buyer.profile.update') }}" method="POST" enctype="multipart/form-data" class="grid grid-cols-1 lg:grid-cols-3 gap-6">
        @csrf
        ...
        <div class="lg:col-span-2 space-y-6">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                    ...
                    <div>
                        ...
                        <input type="text" id="fullname" name="fullname" value="{{ old('fullname', $profile->fullname) }}" required 
                               class="w-full bg-slate-950 border border-slate-800 rounded-xl py-3 px-4 text-sm text-slate-200 focus:outline-none focus:border-emerald-500 focus:ring-1 focus:ring-emerald-500 transition duration-200">
                        @error('fullname')
            ...
            <!-- Save Changes Button -->
            <div>
                <button type="submit" class="water-wave-btn w-full py-3.5 bg-gradient-to-r from-emerald-500 to-teal-600 hover:from-emerald-400 hover:to-teal-500 text-white text-sm font-bold tracking-wide rounded-xl shadow-lg shadow-emerald-500/20 hover:shadow-emerald-500/30 transition duration-200">
        ...
        <div class="lg:col-span-1 space-y-6 h-fit">
            <div class="glass-panel p-6 rounded-3xl flex flex-col items-center justify-center text-center relative group">
                ...
                <div class="relative w-36 h-36 rounded-full overflow-hidden shadow-inner border border-slate-800 bg-slate-950/60 mb-6 flex items-center justify-center">
                    ...
                    @else
                        <div id="avatar-placeholder" class="w-full h-full bg-gradient-to-tr from-emerald-500 to-teal-600 flex items-center justify-center font-bold text-white text-4xl shadow-inner uppercase">
